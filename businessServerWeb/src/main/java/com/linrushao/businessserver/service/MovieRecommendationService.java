...
public interface MovieRecommendationService {
    ...
    //混合推荐
    List<Recommendation> getHybridRecommendations(UserRecommendationForm request);

    // 混合推荐算法【获取ALS离线电影推荐结果+获取当前用户的实时推荐】
    List<Recommendation> findHybridRecommendations(int Uid);

    //协同过滤推荐【用户电影矩阵】【UserRecs表】【用户获取ALS算法中用户推荐矩阵】
    List<Recommendation> findUserCFRecs(int uid);

    //实时推荐结果【StreamRecs表】
    List<Recommendation> findStreamRecs(int uid);

    // 协同过滤推荐【用户电影矩阵】
    List<Recommendation> getUserCFilteringRecommendations(UserRecommendationForm request);
    ...
    List<Recommendation> getContentBasedMoreLikeThisRecommendations(MovieMidRecommendation request);

    //基于内容的推荐算法[moreLikeThisQuery实现基于内容的推荐]
    List<Recommendation> findContentBasedMoreLikeThisRecommendations(int mid);

    // 协同过滤推荐【电影相似性】
    List<Recommendation> findMovieCFRecs(int mid);

    // 协同过滤推荐【电影相似性】
    List<Recommendation> getCollaborativeFilteringRecommendations(MovieMidRecommendation request);
}
